...
const handler = {
    async exec({ m, args }) {
        ...
        if (args.length < 3) {
            ...
            try {
                let _fetch = await fetch(`https://fierce-brushlands-90323.herokuapp.com/translate/google?lang_from=${lang[from].toLowerCase()}&lang_target=${lang[to].toLowerCase()}&text=${_text}`, { mode: 'no-cors', timeout: 0 });
